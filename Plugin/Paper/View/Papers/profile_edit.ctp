<div class="paper form">
	...
	<fieldset>
		...
		echo $this->Form->input('Paper.submittedfile', 
			array(
				'type' => 'file', 'label' => __('Paper File'), 'div' => 'clearfix',
				'after' => '<span class="help-block">' . 
					$this->Html->link(
						__('Download Current File'),
						array('plugin' => false, 'profile' => true, 'controller' => 'papers', 'action' => 'downloadPaper/'.$this->request->data['Paper']['id'])
					)  . '<br>' . 
					__('Only Word 2003 (.doc) format file is allowed. Maximum allowed file size: %d mb<br> Remember, it\'s mandatory to follow the %s.',
					$uploadMb, $mod ) .
				'</span>'
			)
		);
		...
		<fieldset>
			<legend><?php print __('Authors and Co-Authors'); ?></legend>
				...
				echo $this->Form->hidden('Paper.author_name',array('value' => $profile['Profile']['name']));
				...
				$askForExemptionAuthor = $this->Form->radio('Paper.ask_for_exemption',
					array(
				    	0 => null
					),
					array(
				    	'legend' => false,
						'label' => false,
						'value' => (isset($this->request->data['Paper']['ask_for_exemption']) && $this->request->data['Paper']['ask_for_exemption'] == 0) ? 0 : null
					)
				);
			    ...
			    print $this->Html->div('clearfix required',
					$this->Html->div('inline-inputs',
							...
							" &nbsp;&nbsp;&nbsp;&nbsp; $askForExemptionAuthor " . __('Ask for exemption') . 
				...
				for($i=1;$i<=4;$i++){
					...
					$askForExemption = $this->Form->radio('Paper.ask_for_exemption',
						...
						array(
					    	'legend' => false,
							'label' => false,
							'value' => (isset($this->request->data['Paper']['ask_for_exemption']) && $this->request->data['Paper']['ask_for_exemption'] == $i) ? $i : null
						)
					);
				...
				print $this->Form->label('', '&nbsp;') . $this->Form->radio('Paper.ask_for_exemption',
					...
					array(
				    	'legend' => false,
						'value' => (isset($this->request->data['Paper']['ask_for_exemption']) && $this->request->data['Paper']['ask_for_exemption'] === -1) ? -1 : null
					)
